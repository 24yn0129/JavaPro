...
public class ExamTest {
    ...
    public static void main(String[] args) {
        ...
        Examinee[] examineeAry = {
            ...
            new Examinee("高田昇", 28, "評価無し")
        };

        // ここから下が斜線部分に追加するコードです
        System.out.println("■■■統計情報■■■");
        System.out.println("受験者数：" + examineeAry.length + "名");

        double totalScore = 0;
        for (int i = 0; i < examineeAry.length; i++) {
            totalScore += examineeAry[i].getScore();
        }

        double averageScore = totalScore / examineeAry.length;
        System.out.println("筆記試験の平均：" + averageScore + "点");
    }
}